...
"""
MAME Smart ROM Sorter — GUI (Tkinter) + CLI v4.76 (Smart Link Update)
...
Base: v4.75c
v4.76 Changes:
    - ENGINE: Added 'Smart Copy' using hard links to instantly transfer 
      files on the same drive with zero disk space usage.
"""
...
class MameSorter:
    ...
    def _extract_machine_data(self, elem) -> MachineData:
        ...
        # FIX 1: Use 'merge' attribute if available for physical file name mapping
        chds = [(d.get("merge") or d.get("name")) for d in elem.findall("disk") if d.get("name")]
    ...
    def resolve_dependencies(self, games: List[MachineData]) -> Tuple[Set[str], Set[str], Set[str]]:
        ...
        while check_queue:
            ...
            # FIX 2: Pass down context so the copy engine knows where to look for Merged CHDs
            for chd in m.chds:
                root_parent = self._get_root_parent(m.name)
                required_chds.add((chd, m.name, m.cloneof, m.romof, root_parent))
...
def copy_assets(
    rom_list: List[str], bios_list: List[str], chd_list: List[Any], sample_list: List[str],
    ...
    # --- SMART COPY HELPER ---
    def _smart_copy(src: Path, dst: Path) -> bool:
        """Attempts a hard link if on the same drive, falls back to physical copy. Returns True if linked."""
        # Check if drives match (Windows specific: .drive returns 'C:', 'E:', etc.)
        if src.drive and dst.drive and src.drive.upper() == dst.drive.upper():
            try:
                os.link(src, dst)
                return True
            except OSError:
                pass # Fallback to physical copy if link fails (e.g., exFAT drive or cross-device error)
        
        # Standard physical copy
            ...
            if source.exists():
                try:
                    ...
                    if not dest_file.exists():
                        is_linked = _smart_copy(source, dest_file)
                        if is_linked:
                            linked_count += 1
                            copied_assets.append(f"[LINK] ROM/BIOS: {source.name}")
                        else:
                            copied_count += 1
                            copied_assets.append(f"[COPY] ROM/BIOS: {source.name}")
                    else:
                        copied_count += 1 # Already exists
                    found = True
                    break
                except OSError as e:
    ...
    # Dynamic Waterfall Search for Merged/Split CHDs
    for chd_data in chd_list:
        if isinstance(chd_data, tuple):
            chd_name, m_name, cloneof, romof, root_parent = chd_data
            chd_filename = f"{chd_name}.chd"

            # Build search paths in order of MAME's preference (Child -> Parent -> Root)
            search_folders = [m_name]
            if cloneof: search_folders.append(cloneof)
            if romof: search_folders.append(romof)
            if root_parent: search_folders.append(root_parent)

            # Remove duplicates while keeping the priority order
            seen = set()
            search_folders = [x for x in search_folders if not (x in seen or seen.add(x))]

            source_chd = None
            actual_folder = None

            for folder in search_folders:
                test_path = rom_dir / folder / chd_filename
                if test_path.exists():
                    source_chd = test_path
                    actual_folder = folder
                    break

            if source_chd:
                dest_folder = out_rom_dir / actual_folder
                dest_folder.mkdir(exist_ok=True, parents=True)
                try:
                    dest_file = dest_folder / source_chd.name
                    if not dest_file.exists():
                        is_linked = _smart_copy(source_chd, dest_file)
                        if is_linked:
                            linked_count += 1
                            copied_assets.append(f"[LINK] CHD: {actual_folder}/{chd_filename}")
                        else:
                            copied_count += 1
                            copied_assets.append(f"[COPY] CHD: {actual_folder}/{chd_filename}")
                    else:
                        copied_count += 1
                except OSError as e:
                    if e.errno == errno.ENOSPC:
                ...
                missing_assets.append(f"CHD: {m_name}/{chd_filename} (Not found in {search_folders})")
        else:
            # Fallback for old string format logic
            chd_rel_path = chd_data
            ...
            if source_chd.exists():
                ...
                try:
                    ...
                    if not dest_file.exists():
                        ...
                        if is_linked:
                            linked_count += 1
                            copied_assets.append(f"[LINK] CHD: {chd_rel_path}")
                        else:
                            copied_count += 1
                            copied_assets.append(f"[COPY] CHD: {chd_rel_path}")
                    else:
                        copied_count += 1
                except OSError as e:
                    if e.errno == errno.ENOSPC:
        ...
        for sample_name in sample_list:
            ...
            if source_sample.exists():
                try:
                    ...
                    if not dest_file.exists():
                        is_linked = _smart_copy(source_sample, dest_file)
                        if is_linked:
                            linked_count += 1
                            copied_assets.append(f"[LINK] Sample Pack: {source_sample.name}")
                        else:
                            copied_count += 1
                            copied_assets.append(f"[COPY] Sample Pack: {source_sample.name}")
                    else:
                        copied_count += 1
                except OSError as e:
                    if e.errno == errno.ENOSPC:
    ...
    total_success = copied_count + linked_count
    if log_cb: log_cb(f"✅ Operation complete. {total_success} assets processed ({linked_count} instantly hard-linked, {copied_count} physically copied).")
